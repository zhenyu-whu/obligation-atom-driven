...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| T-001 | AC-001 | npm test -- admin-items.route.contract.test.ts | admin-items.route.contract.test.ts::admin create contract | route/API contract | RSI-001 OCM-001 | yes | in-process route handler with test auth token | yes | npm test -- admin-items.route.contract.test.ts | route returns 404 before implementation | observed missing handler | npm test -- admin-items.route.contract.test.ts | green-passed | none | test-results/security-data-behavior/AC-001/T-001/ | command.log, ledger.json, route contract log | test-results/security-data-behavior/AC-001/T-001/ledger.json | yes | GA-0001 | required behavior | no scope expansion |
| T-002 | AC-001 | npm test -- admin-items.repository.integration.test.ts | admin-items.repository.integration.test.ts::persists once | DB/integration | RSI-001 OCM-001 SBM-001 | yes | transaction-scoped test database | yes | npm test -- admin-items.repository.integration.test.ts | row not inserted before implementation | observed zero rows | npm test -- admin-items.repository.integration.test.ts | green-passed | T-001 | test-results/security-data-behavior/AC-001/T-002/ | command.log, ledger.json, DB assertion log | test-results/security-data-behavior/AC-001/T-002/ledger.json | yes | GA-0001 | required behavior | no scope expansion |
| T-003 | AC-001 | npm test -- admin-items.security.test.ts | admin-items.security.test.ts::rejects unauthenticated write | security/negative | RSI-001 OCM-002 SBM-002 | yes | transaction-scoped test database, no auth token | yes | npm test -- admin-items.security.test.ts | unauthenticated write succeeds before guard | observed unexpected 201 | npm test -- admin-items.security.test.ts | green-passed | T-001 T-002 | test-results/security-data-behavior/AC-001/T-003/ | command.log, ledger.json, negative auth log | test-results/security-data-behavior/AC-001/T-003/ledger.json | yes | GA-0002 | required behavior | no scope expansion |
| T-004 | AC-001 | npx playwright test admin-items.spec.ts | admin-items.spec.ts::admin default path creates item | browser E2E | RSI-001 OCM-001 OCM-002 | yes | local app server and test database | yes | npx playwright test admin-items.spec.ts | item not visible before implementation | observed missing item | npx playwright test admin-items.spec.ts | green-passed | T-001 T-002 T-003 | test-results/security-data-behavior/AC-001/T-004/ | command.log, ledger.json, browser trace and DB readback | test-results/security-data-behavior/AC-001/T-004/ledger.json | yes | GA-0001 GA-0002 | required behavior | no scope expansion |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| AC-001 | T-001 T-002 T-003 T-004 | admin-items.route.contract.test.ts; admin-items.repository.integration.test.ts; admin-items.security.test.ts; admin-items.spec.ts | npm test -- admin-items.route.contract.test.ts && npm test -- admin-items.repository.integration.test.ts && npm test -- admin-items.security.test.ts && npx playwright test admin-items.spec.ts | admin creation is authorized, persisted once, and unauthorized writes do not mutate DB | public API status/body, DB row count, and authorization denial | in-process route handler, transaction-scoped DB, local app server | local/CI | private helper calls and DOM structure | deposited |
