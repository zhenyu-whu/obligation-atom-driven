...
- 归档前必须确认每个已完成 AC 已执行 AC evidence audit，change-stabilizer 已执行全量 Testing Quality Core final audit，且只读 final-reviewer 已返回 pass：逐项检查 Test Evidence Matrix、Regression Test Deposit、canonical evidence directory、fixed command 结果、Evidence Status 和 execution evidence。不得跳过 Testing Quality Core；若 final-reviewer 返回 blocker 或缺少 final-reviewer 报告，不得归档。
