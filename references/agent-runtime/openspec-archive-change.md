...
- 两个生产 schema 都必须检查 `Runtime Acceptance Index`、AC-local contract 和 `Verification Appendix` 六张 runtime/test 矩阵能回链到已完成任务、fixed commands、canonical evidence directories、verification evidence 和 execution evidence；Test ID 必须匹配 exact `T-[0-9]{3}`，不得带 AC 编号、slug、名称或字母后缀。无 runtime 行为的 runtime detail matrix 只能保留 source/scope-backed `Not applicable` 最小行。每个 completed Test ID 的 canonical evidence directory 必须至少包含 `command.log` 或 runner/CI result/report；本地 apply evidence 还必须包含 evidence harness 生成的 `ledger.json`，等价 CI evidence 必须包含可审计 artifact index。`deposited` required behavior 必须通过当前 worktree 的 execution evidence 证明通过，或证明 regression command 与已通过的 fixed command 等价。固定命令不得用 `pnpm test* -- ...` 透传 file/filter 作为 Test ID selector。归档 evidence ownership 以 apply-stage harness 或 CI artifact collector 的 canonical 输出为准；测试侧 artifact 可通过 runner attachment/report、框架 output directory 或 harness 配置目录被收集和索引。归档 final gate 中不得存在 `planned`、`blocked`、缺失 execution evidence 或未闭合 deposit 的 required behavior。
- 归档前必须对每个已完成 AC 执行 AC evidence audit，并执行全量 Testing Quality Core final audit：逐项检查 Test Evidence Matrix、Regression Test Deposit、canonical evidence directory、fixed command 结果、Evidence Status 和 execution evidence。不得跳过 Testing Quality Core。
...
- 只有遇到冲突或高风险状态时才向用户确认，包括但不限于：多个候选 change、归档目标目录已存在、delta spec 与主 spec 无法明确合并、存在未完成任务、coverage 表存在 orphan `GA-####` / `SI-###`、proof/execution evidence 不足、canonical `command.log` 或 runner/CI result/report 缺失、harness-owned `ledger.json` 或等价 CI artifact index 缺失、Regression Test Deposit 缺失或不可信、`deposited` 只登记计划路径、命令失败、校验失败，或归档会覆盖/删除非目标文件。
