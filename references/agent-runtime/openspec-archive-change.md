...
- 两个 production schema 的新格式采用 Delivery Plane + JSON Trace Plane 布局：归档检查必须从 artifact 主体读取交付契约，从短 `## Trace Appendix` pointer、`trace/manifest.json` 和对应 JSON trace 读取 coverage、projection、reconciliation 和 alignment gate。
...
- `openspec-results/<change-slug>/apply-result.md` 必须包含 `Regression Reconciliation` section，记录全量回归命令、退出状态或环境 blocker、每个失败测试的分类、分类依据、处理动作和最终状态；该 section 是归档前调和历史测试假设与当前 OpenSpec 真相的必需 evidence。
...
- Regression Reconciliation Gate 中任一 `current-change-regression`、`current-proof-instability`、`artifact-consistency-blocker` 未解决时不得归档成功；`pre-existing-failure` 必须有 baseline 证据，`flaky-or-environmental` 必须已有稳定化结果或明确环境 blocker，`historical-test-superseded` 和 `historical-test-fixture-stale` 必须记录对应处理动作和当前 contract basis。
