...
- `trace/tasks.trace.json` 不得包含旧测试矩阵、schema source coverage 身份、evidence/apply path、deposit status 或 legacy proof/preserve 字段。若 validator warning 指出 tasks 文本包含测试命令或测试文件，归档前必须确认 reviewer/preflight 已处理且未形成 proof/test/evidence-only task。
...
- 只有遇到冲突或高风险状态时才向用户确认，包括但不限于：多个候选 change、归档目标目录已存在、delta spec 与主 spec 无法明确合并、存在未完成 checkbox、coverage 表存在 orphan `GA-####` / `SI-###`、runtime trace 缺失、runtime fact 重复或缺少 source/scope/default-path/no-scope 字段、tasks/verification trace 引用未定义 runtime fact、tasks trace 引用未定义 spec scenario 或 design detail、tasks trace 旧字段残留、tasks validator warning 未被 reviewer/preflight 处理、spec/design/runtime closure 不闭合、required / preserve runtime fact 缺少 verification Proof Slice、required Proof Slice 缺少最终 proof result、durable slice 缺测试或测试文件未落在 planned directory 下、non-durable slice 缺 evidence 或误入 proof-test-results、存在未解决 blocker、命令失败、校验失败，或归档会覆盖/删除非目标文件。
