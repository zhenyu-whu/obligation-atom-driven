...
8. 两个生产 schema 的 common preflight 必须索引：
   ...
   - evidence ledger targets：fixed commands、canonical `test-results/<change-slug>/<AC-ID>/<Test-ID>/` artifacts、browser/rendered artifacts、API/DB/job/storage/log/audit facts、default-production-path proof。固定命令不得用 `pnpm test* -- ...` 透传 file/filter 作为 Test ID 级 selector；需要 selector 时使用 runner 直接命令、`pnpm exec vitest/playwright ...` 或专用 script。
...
10. 两个生产 schema 的仓库必须提供 `openspec/agent-runtime/scripts/validate_tasks_quality.py`、`openspec/agent-runtime/scripts/write_evidence_ledger.py` 和 `openspec/schemas/shared/evidence-ledger.schema.json`。在分派 worker 或开始实现前必须运行 `python3 openspec/agent-runtime/scripts/validate_tasks_quality.py openspec/changes/<change-slug>/tasks.md`。普通校验会读取所有已 `deposited` 的 ledger；若报告 error，先修订 artifacts 或对应 evidence，不得用实现绕过 Testing Quality Core 缺口。若脚本或 schema 缺失，先同步 bundled schema/runtime；缺失本身是 schema sync blocker。
...
8. **Gate 8 / 任务级 TDD 与 proof 检查**：worker 应优先按当前 AC-local `Test IDs`、任务 `Test IDs`、`Proof:`、test layer code requirements 和 regression deposit（若适用）建立或补齐对应验证。实现完成前 proof 应能失败或暴露缺口；实现后必须通过 AC-owned Test ID 在 `Test Evidence Matrix` 中定义的 fixed command，并在 canonical evidence directory 产出 `command.log` 和符合 `openspec/schemas/shared/evidence-ledger.schema.json` 的 `ledger.json`。当前 AC final verification / acceptance checkbox 勾选前必须运行 `python3 openspec/agent-runtime/scripts/validate_tasks_quality.py openspec/changes/<change-slug>/tasks.md --ac <AC-###> --evidence` 并修复本 AC 所有 ledger/deposit error。用户可见操作必须执行当前 schema 中定义的 runtime interaction / operation matrix proof；presence-only、static-only、API-only、implementation-detail 或 broad command proof 不得单独支撑勾选。
...
10. **Gate 10 / 完成判定**：只有当 `tasks.md` 全部 checkbox 完成、三张 coverage 表、`Runtime Acceptance Index`、每个 AC-local contract、`Verification Appendix` 六张 runtime/test 矩阵、required evidence、canonical evidence directories、TDD red/green evidence、evidence ledger、每个已完成 AC 的 `validate_tasks_quality.py --ac <AC-###> --evidence`、`Regression Test Deposit`、`validate_tasks_quality.py --final`、以及独立 `reviewer` subagent 复核结论都能回链到已完成任务和 proof，且 reviewer 返回 pass，才可称为 ready for archive。
...
10. 必须明确告知 worker：完成任务时要执行 AC-owned Test IDs 在 `Test Evidence Matrix` 中定义的 fixed command，并在 `test-results/<change-slug>/<AC-ID>/<Test-ID>/` 提供 `command.log` 和符合 `openspec/schemas/shared/evidence-ledger.schema.json` 的 `ledger.json`，且 `<Test-ID>` 必须匹配 `T-[0-9]{3}`。ledger 必须使用 `acId` 字段并记录 canonical schema 要求的 behavior contract、assertion oracle、expected/observed red failure、fixed/red/green/regression command、red/green/regression result、cwd、exit code、started/finished 时间、artifacts 数组、default-path facts、fixture boundary、TDD status 和 not-applicable reason；`tddStatus` 必须使用 `green-passed` 等 schema 枚举，禁止 `red-green`。优先使用 `python3 openspec/agent-runtime/scripts/write_evidence_ledger.py openspec/changes/<change-slug>/tasks.md --test-id <T-###>` 生成 ledger，再补入实际命令结果。当前 AC final verification / acceptance checkbox 勾选前，worker 必须运行 `python3 openspec/agent-runtime/scripts/validate_tasks_quality.py openspec/changes/<change-slug>/tasks.md --ac <AC-###> --evidence` 并修复本 AC 所有 error；AC-005/final proof 只能消费已经通过各自 AC evidence gate 的前置 ledgers，不得把 final gate 作为前置 AC ledger 的首次 canonicalization 检查。`/tmp`、未复制的 runner 默认输出、agent 当场手工截图或口述路径不得作为最终 evidence。两个生产 schema worker 都必须按 `Test Layer Plan`、test layer code requirements、TDD red/green fields 和 `Regression Test Deposit` 落实分层测试与永久回归测试；若 deposit 标记 `not-applicable` 或 `blocked`，必须给出 source/scope-backed 理由。
