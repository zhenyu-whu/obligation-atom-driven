...
10. 两个生产 schema 的仓库必须提供 `openspec/agent-runtime/scripts/validate_tasks_quality.py`。`openspec/agent-runtime/scripts/write_evidence_ledger.py` 和 `openspec/schemas/shared/evidence-ledger.schema.json` 用于可选 audit ledger。在分派 worker 或开始实现前必须运行 `python3 openspec/agent-runtime/scripts/validate_tasks_quality.py openspec/changes/<change-slug>/tasks.md`。普通校验会检查所有已 `deposited` 的 current worktree evidence，已声明或已存在的 ledger 只做一致性校验；若报告 error，先修订 artifacts 或对应 evidence，不得用实现绕过 Testing Quality Core 缺口。若 validator 缺失，先同步 bundled schema/runtime；缺失本身是 schema sync blocker。
...
8. **Gate 8 / 任务级 TDD 与 proof 检查**：worker 应优先按当前 AC-local `Test IDs`、任务 `Test IDs`、`Proof:`、test layer code requirements 和 regression deposit（若适用）建立或补齐对应验证。实现完成前 proof 应能失败或暴露缺口；实现后必须通过 AC-owned Test ID 在 `Test Evidence Matrix` 中定义的 fixed command，并在 canonical evidence directory 产出 `command.log` 或 runner/CI result/report；可选 `ledger.json` 只能作为 apply/reviewer/archive audit receipt。当前 AC final verification / acceptance checkbox 勾选前必须运行 `python3 openspec/agent-runtime/scripts/validate_tasks_quality.py openspec/changes/<change-slug>/tasks.md --ac <AC-###> --evidence` 并修复本 AC 所有 execution evidence / deposit error。用户可见操作必须执行当前 schema 中定义的 runtime interaction / operation matrix proof；presence-only、static-only、API-only、implementation-detail 或 broad command proof 不得单独支撑勾选。
