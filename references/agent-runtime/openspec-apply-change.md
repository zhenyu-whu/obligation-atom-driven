...
8. 两个生产 schema 的 common preflight 必须索引：
   ...
   - execution evidence targets：fixed commands、evidence harness 生成的 canonical `openspec-results/<change-slug>/<AC-ID>/<Test-ID>/` artifacts、harness-owned `ledger.json`、browser/rendered artifacts、API/DB/job/storage/log/audit facts、runner/CI result/report 和 default-production-path proof。固定命令不得用 `pnpm test* -- ...` 透传 file/filter 作为 Test ID 级 selector；需要 selector 时使用 runner 直接命令、`pnpm exec vitest/playwright ...` 或专用 script。
...
8. **Gate 8 / 任务级验证与 proof 检查**：worker 应优先按当前 AC-local `Test IDs`、任务 `Test IDs`、`Proof:`、test layer code requirements 和 regression deposit（若适用）建立或补齐对应验证。完成实现与验证后必须通过 apply-stage evidence harness 运行同一 Test ID 的唯一 `Fixed Command`，在 canonical evidence directory 产出 `command.log`、harness-owned `ledger.json`、runner/CI result/report 或被 harness 索引的等价 artifacts，并把 `Evidence Status`、`Evidence Produced` 和 deposit 状态更新回 `tasks.md`。canonical `ledger.json`、`command.log` 和 evidence path 由 harness 管理；测试代码关注行为断言，并通过 runner artifact、测试框架 output directory 或 harness 注入的临时 artifact directory 暴露截图、trace、DOM/API/DB/job/log facts，再由 evidence harness 复制和索引。当前 AC final verification / acceptance checkbox 勾选前必须执行 AC evidence audit：对照当前 AC 的 Test Evidence rows、Regression Test Deposit、canonical evidence directory、harness ledger 和 fixed command 结果，修复本 AC 所有 execution evidence / deposit 缺口。用户可见操作必须执行当前 schema 中定义的 runtime interaction / operation matrix proof；presence-only、static-only、API-only、implementation-detail 或 broad command proof 不得单独支撑勾选。
...
2. worker 是当前 AC 的唯一执行者，拥有 current-change-scoped repair authority。只要修复仍在当前 change scope 内，worker 可以修改实现、测试、tasks artifact、`Test Evidence Matrix`、`Regression Test Deposit`、evidence harness，并重跑同一个 fixed command。
...
- worker 报告的 evidence harness 输出，包括每个 Test ID 的 `command.log`、harness-owned `ledger.json` 或等价 runner/CI artifact index，并说明 runner artifacts 如何被 harness/CI collector 收集。
- worker 执行的验证命令及结果。
