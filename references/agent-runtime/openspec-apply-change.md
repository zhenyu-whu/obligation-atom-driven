...
8. **Gate 8 / 验收强度与 evidence ledger 检查**：每个 AC section 的 final verification task 必须完成 `Primary Proof` 和 `Required Evidence`，并产出 evidence ledger 记录。用户可见 AC 需要 browser-rendered evidence；backend/data/worker/storage/security AC 需要 API、DB、queue/job、asset、log、audit 或 authorization facts。
...
1. 用户明确要求执行或继续执行 `openspec-apply-change` 技能时，该请求即代表用户显式授权并要求按本节使用 `worker` subagent、delegation 和 parallel agent work，除非用户在同一请求中明确禁止 subagent。
2. 对每个包含未完成任务的 AC section，必须分别创建一个 `worker` subagent 进行开发，除非用户明确禁止 subagent 或运行环境不可用。
3. 每个 worker 只负责自己 AC section 内的任务和该 section 的验收 proof。
4. 启动 worker 时默认不要 fork 完整对话历史；使用显式任务包传递必要上下文。只有当该章节必须依赖当前对话中尚未写入文件的决策时，才允许 fork。
5. 启动 worker 时必须传入：
...
6. 必须明确告知 worker：实现前必须通过 linked `GA-####` 定点读取原始 source docs 相关片段，并在最终回复中列出读取的 source windows；若发现 context 冲突、任务边界不清、trace 缺失、proof 不可执行、task 弱于 atoms/spec/design/source docs，必须停止猜测并标明 blocker。
7. 必须明确告知 worker：它不是唯一开发者，不得回滚或覆盖其他 agent / 用户的改动；遇到重叠文件或冲突风险必须适配现有改动并在最终回复中说明。
8. 必须明确告知 worker：完成任务时要提供 evidence ledger 条目，包括命令、截图/DOM、API/DB/job/storage/log/audit facts 或 default-path proof；只报告“测试通过”不足以支撑勾选。

## 主 Agent 职责

1. 主 agent 负责 orchestration：选择 change、读取 status / instructions、解析 context、按 AC section 分派 worker、协调 ownership、等待结果、统一审查改动、执行最终 coverage/proof audit、汇总证据。
...
4. 统一 audit 至少确认：
   ...
   - 未引入明显跨 AC 冲突。
5. 若验收 proof 失败揭示 proposal/spec/design/tasks 与 obligation atoms 不一致，主 agent 必须暂停代码修复并提出 artifact 更新；不得用实现绕过 artifact mismatch。
