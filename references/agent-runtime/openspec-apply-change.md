...
8. 两个生产 schema 的 common preflight 必须索引：
   ...
   - test evidence matrix：Test ID、owning AC ID、fixed command、test file/name、layer、covered row IDs、`Default Path Level`（旧表可兼容 `Default Path?`）、fixture boundary、must-fail-before-implementation、Red Command、Expected Red Failure、Observed Red Failure、Green Command、TDD Status、Requires Tests Passed、canonical evidence directory、evidence produced、CI runnable 标记和可选 ledger file；每个 Test ID 必须匹配 exact `T-[0-9]{3}`，不得带测试名称、AC 编号、slug 或字母后缀。tasks 生成阶段只定义计划契约，新增/变更 required behavior 默认应停留在 `red-required` 和 apply-pending observed/evidence 字段；apply 阶段负责运行 red/green 并更新这些字段。
   ...
   - execution evidence targets：fixed commands、canonical `test-results/<change-slug>/<AC-ID>/<Test-ID>/` artifacts、browser/rendered artifacts、API/DB/job/storage/log/audit facts、runner/CI result/report、default-production-path proof，以及 optional audit ledger。固定命令不得用 `pnpm test* -- ...` 透传 file/filter 作为 Test ID 级 selector；需要 selector 时使用 runner 直接命令、`pnpm exec vitest/playwright ...` 或专用 script。
...
10. 两个生产 schema 的仓库必须提供 `openspec/agent-runtime/scripts/validate_tasks_quality.py`。`openspec/agent-runtime/scripts/write_evidence_ledger.py` 和 `openspec/schemas/shared/evidence-ledger.schema.json` 用于可选 audit ledger。在分派 worker 或开始实现前必须运行默认 plan gate：`python3 openspec/agent-runtime/scripts/validate_tasks_quality.py openspec/changes/<change-slug>/tasks.md`。plan gate 检查测试计划、默认路径和回归沉淀契约，但不要求 apply-owned red/green evidence 已存在；若报告 error，先修订 artifacts。已声明或已存在的 ledger 只在 evidence/final gate 中成为严格一致性约束。若 validator 缺失，先同步 bundled schema/runtime；缺失本身是 schema sync blocker。
...
5. **Gate 5 / Runtime model 覆盖检查**：`Verification Appendix` 中每个 mandatory row 必须有 basis、scope role、provider/consumer ownership、AC ID、Test ID 和 no-scope-expansion check；每个 Test ID 必须绑定具体 covered row IDs，只能归属一个 exact `AC-###`，并且必须匹配 exact `T-[0-9]{3}`。fixed command、test file/name、evidence directory、execution artifacts、optional ledger file、fixture boundary、Requires Tests Passed 和 CI runnable 状态只能在 `Test Evidence Matrix` 中定义。
...
8. **Gate 8 / 任务级 TDD 与 proof 检查**：worker 应优先按当前 AC-local `Test IDs`、任务 `Test IDs`、`Proof:`、test layer code requirements 和 regression deposit（若适用）建立或补齐对应验证。写 production code 前必须先创建或补齐测试并运行 red command；red 失败必须匹配 `Expected Red Failure`，然后把 `Observed Red Failure` 和 red evidence path 更新回 `tasks.md`。实现后必须通过同一 Test ID 的 green/fixed command，在 canonical evidence directory 产出 `command.log` 或 runner/CI result/report，并把 `TDD Status`、`Evidence Produced` 和 deposit 状态更新回 `tasks.md`；可选 `ledger.json` 只能作为 apply/reviewer/archive audit receipt。当前 AC final verification / acceptance checkbox 勾选前必须运行 `python3 openspec/agent-runtime/scripts/validate_tasks_quality.py openspec/changes/<change-slug>/tasks.md --ac <AC-###> --evidence` 并修复本 AC 所有 execution evidence / deposit error。用户可见操作必须执行当前 schema 中定义的 runtime interaction / operation matrix proof；presence-only、static-only、API-only、implementation-detail 或 broad command proof 不得单独支撑勾选。
...
10. **Gate 10 / 完成判定**：只有当 `tasks.md` 全部 checkbox 完成、三张 coverage 表、`Runtime Acceptance Index`、每个 AC-local contract、`Verification Appendix` 六张 runtime/test 矩阵、required evidence、canonical evidence directories、TDD red/green evidence、execution evidence、optional audit ledger（若声明或存在）、每个已完成 AC 的 `validate_tasks_quality.py --ac <AC-###> --evidence`、`Regression Test Deposit`、`validate_tasks_quality.py --final`、以及独立 `reviewer` subagent 复核结论都能回链到已完成任务和 proof，且 reviewer 返回 pass，才可称为 ready for archive。`--final` 下不得存在 `Pending apply` / `red-required` / 未落盘 observed red 或 green evidence 的 required behavior；声明了 `Ledger File` 但文件缺失必须视为 blocker。
...
7. 启动 worker 时必须传入：
   ...
   - 与该 AC 相关的 execution evidence targets、`Test Evidence Matrix` 定义的 canonical `test-results/<change-slug>/<AC-ID>/<Test-ID>/` evidence storage contract、optional audit ledger（若声明），以及 final verification / acceptance checkbox ID。
   - 允许修改的代码范围或模块边界。
...
10. 必须明确告知 worker：完成任务时要执行 AC-owned Test IDs 在 `Test Evidence Matrix` 中定义的 fixed command，并在 `test-results/<change-slug>/<AC-ID>/<Test-ID>/` 提供 `command.log` 或 runner/CI result/report，且 `<Test-ID>` 必须匹配 `T-[0-9]{3}`。可选 `ledger.json` 只作为 apply/reviewer/archive audit receipt；若生成，必须使用 `acId` 字段并记录 canonical schema 要求的 behavior contract、assertion oracle、expected/observed red failure、fixed/red/green/regression command、red/green/regression result、cwd、exit code、started/finished 时间、artifacts 数组、default-path facts、fixture boundary、TDD status 和 not-applicable reason；`tddStatus` 必须使用 `green-passed` 等 schema 枚举，禁止 `red-green`。可选使用 `python3 openspec/agent-runtime/scripts/write_evidence_ledger.py openspec/changes/<change-slug>/tasks.md --test-id <T-###>` 生成 audit ledger，但不能替代实际测试执行证据。当前 AC final verification / acceptance checkbox 勾选前，worker 必须运行 `python3 openspec/agent-runtime/scripts/validate_tasks_quality.py openspec/changes/<change-slug>/tasks.md --ac <AC-###> --evidence` 并修复本 AC 所有 error；AC-005/final proof 只能消费已经通过各自 AC evidence gate 的前置 evidence，不得把 final gate 作为前置 AC 执行证据的首次 canonicalization 检查。`/tmp`、未复制的 runner 默认输出、agent 当场手工截图或口述路径不得作为最终 evidence。两个生产 schema worker 都必须按 `Test Layer Plan`、test layer code requirements、TDD red/green fields 和 `Regression Test Deposit` 落实分层测试与永久回归测试；若 deposit 标记 `not-applicable` 或 `blocked`，必须给出 source/scope-backed 理由。
...
4. 主 agent 启动 reviewer 时必须传入：change 名称、schema 名称、`contextFiles`、proposal/specs/design/tasks 路径、所有 worker 最终报告、worker 报告的改动范围、所有 AC/Test ID/execution evidence 路径、optional audit ledger 路径（若声明或存在）、fixed commands、Regression Test Deposit 状态，以及 reviewer 需要复核的 coverage/runtime/test 矩阵路径。主 agent 不得为了准备 reviewer 输入而自行审查 diff、打开 evidence 或 optional ledger、重跑验证命令或预先判断 worker 结果是否可信。
5. reviewer 负责独立复核：检查代码 diff、artifacts、tasks checkbox、coverage/runtime/test 矩阵、execution evidence、optional audit ledger（若声明或存在）、Regression Test Deposit、固定验证命令结果、跨 AC 集成冲突、默认路径/no-mock 约束、TDD red/green 证据，并运行必要的复核命令，包括每个已完成 AC 的 `python3 openspec/agent-runtime/scripts/validate_tasks_quality.py openspec/changes/<change-slug>/tasks.md --ac <AC-###> --evidence` 和全量 `python3 openspec/agent-runtime/scripts/validate_tasks_quality.py openspec/changes/<change-slug>/tasks.md --final`。
6. reviewer 只输出复核报告和 pass/blocker 结论，不得直接修改代码、artifacts、checkbox、execution evidence、optional ledger 或测试文件。若发现 blocker，主 agent 只汇报 reviewer blocker；不得自行接手修复、替 worker 补 proof、替 reviewer 复验，除非用户在 blocker 汇报后明确要求继续处理。
...
2. 主 agent 必须等待所有已分派 worker 的任务全部完成或明确 blocker；若全部 worker 完成且无 blocker，必须等待 reviewer 自然返回 pass 或 blocker 后，才能做最终汇总。worker 已声明完成的 AC section、checkbox、proof、evidence、fixed command 结果和 optional ledger 视为 worker 输出，reviewer 的 pass/blocker 视为最终复核输出；主 agent 不得再次复核、重测、审查 diff、重新跑验证命令、重新检查 execution evidence / optional ledger、重新执行 coverage/proof audit，或以主 agent 判断覆盖 worker/reviewer 的结论。
...
- worker 报告的每个 AC 的 primary proof、required evidence、canonical evidence directories、execution evidence、optional audit ledger（若有）、截图/DOM/API/DB/job/log/audit 等证据。
- worker 执行的验证命令及结果。
