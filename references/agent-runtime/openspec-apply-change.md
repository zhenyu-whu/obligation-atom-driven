...
10. 两个生产 schema 的仓库必须提供 `openspec/agent-runtime/scripts/validate_tasks_quality.py`。在分派 worker 或开始实现前必须运行默认 plan gate：`python3 openspec/agent-runtime/scripts/validate_tasks_quality.py openspec/changes/<change-slug>/tasks.md`。plan gate 检查测试计划、默认路径和回归沉淀契约，但不要求 apply-owned execution evidence 已存在；若报告 error，先修订 artifacts。若 validator 缺失，先同步 bundled schema/runtime；缺失本身是 schema sync blocker。
...
10. **Gate 10 / 完成判定**：只有当 `tasks.md` 全部 checkbox 完成、三张 coverage 表、`Runtime Acceptance Index`、每个 AC-local contract、`Verification Appendix` 六张 runtime/test 矩阵、required evidence、canonical evidence directories、verification evidence、execution evidence、每个已完成 AC 的 `validate_tasks_quality.py --ac <AC-###> --evidence`、`Regression Test Deposit`、`validate_tasks_quality.py --final`、以及独立 `reviewer` subagent 复核结论都能回链到已完成任务和 proof，且 reviewer 返回 pass，才可称为 ready for archive。`--final` 下不得存在 `planned`、缺失 execution evidence 或未闭合 deposit 的 required behavior。
