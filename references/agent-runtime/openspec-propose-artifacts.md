...
15. Testing Quality Core 不可选：`Test Layer Plan`、`Test Evidence Matrix` 和 `Regression Test Deposit` 必须始终存在。每个 required behavior Test ID 必须有 red/green TDD fields，除非写明 source-backed `not-applicable` 或 `blocked` 理由；不得把 smoke/browser proof 当作 unit/component/API/DB/security 等可低层稳定断言的替代品。
...
17. tasks 必须定义每个 AC 的 evidence ledger expectation，包括 commands、browser/rendered artifacts、API/DB/job/storage/log/audit facts、default-production-path proof。
...
16. Testing Quality Core 不可选：每个 required behavior Test ID 必须在 `Test Layer Plan` 中有 layer decision，在 `Test Evidence Matrix` 中有 red/green TDD fields，并在 `Regression Test Deposit` 中有永久回归沉淀或 scope-backed `not-applicable` 理由。不得把私有 helper、mock 调用次数、非契约 DOM 层级、className、快照全文、`data-testid` 存在、按钮 enabled 或当前实现输出登记为 required behavior 的 primary proof 或永久回归 oracle；不得把 smoke/browser proof 当作 unit/component/API/DB/security 等可低层稳定断言的替代品。
