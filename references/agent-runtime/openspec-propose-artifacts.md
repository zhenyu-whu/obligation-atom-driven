...
17. tasks 必须定义每个 AC 的 execution evidence expectation，包括 commands、browser/rendered artifacts、API/DB/job/storage/log/audit facts、runner/CI result/report 和 default-production-path proof。
...
10. 两个生产 schema 的仓库必须提供 `openspec/agent-runtime/scripts/validate_tasks_quality.py`。生成或修订 `tasks.md` 后必须运行默认 plan gate：`python3 openspec/agent-runtime/scripts/validate_tasks_quality.py openspec/changes/<change-slug>/tasks.md`。若 validator 缺失，先同步 bundled schema/runtime；缺失本身是 schema sync blocker。若报告 error，先修订 `tasks.md`；OpenSpec 结构校验通过不等同于 Testing Quality Core 通过。不要在 propose/tasks 阶段补造执行证据；这些属于 apply 阶段。
