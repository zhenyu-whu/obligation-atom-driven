...
10. 如果仓库存在 `openspec/agent-runtime/scripts/validate_tasks_quality.py`，生成或修订 `tasks.md` 后必须运行 `python openspec/agent-runtime/scripts/validate_tasks_quality.py openspec/changes/<change-slug>/tasks.md`。若报告 error，先修订 `tasks.md`；OpenSpec 结构校验通过不等同于 Testing Quality Core 通过。
