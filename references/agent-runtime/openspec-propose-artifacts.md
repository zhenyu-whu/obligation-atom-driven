...
15. Testing Quality Core 不可选：`Test Layer Plan`、`Test Evidence Matrix` 和 `Regression Test Deposit` 必须始终存在。每个 required behavior Test ID 必须有 red/green TDD fields，除非写明 source-backed `not-applicable` 或 `blocked` 理由；不得把 smoke/browser proof 当作 unit/component/API/DB/security 等可低层稳定断言的替代品。固定命令必须是真正定位该 Test ID 的最小命令，不得用 `pnpm test* -- ...` 透传 file/filter 作为 selector。`Test Evidence Matrix` 必须写明 layer-specific code/harness marker：browser E2E 要真实 browser/page/context + 用户动作/readback，component 要交互式 component harness，route/API、DB、worker、SSE、visual、security、ops 要证明各自 production-compatible boundary；目录名或 `tests/e2e/**` 不能单独决定层级。
...
16. Testing Quality Core 不可选：每个 required behavior Test ID 必须在 `Test Layer Plan` 中有 layer decision，在 `Test Evidence Matrix` 中有 red/green TDD fields，并在 `Regression Test Deposit` 中有永久回归沉淀或 scope-backed `not-applicable` 理由。不得把私有 helper、mock 调用次数、非契约 DOM 层级、className、快照全文、`data-testid` 存在、按钮 enabled 或当前实现输出登记为 required behavior 的 primary proof 或永久回归 oracle；不得把 smoke/browser proof 当作 unit/component/API/DB/security 等可低层稳定断言的替代品。`Test Evidence Matrix` 必须写明 layer-specific code/harness marker：browser E2E 要真实 browser/page/context + 用户动作/readback，component 要交互式 component harness，route/API、DB、worker、SSE、visual、security、ops 要证明各自 production-compatible boundary；目录名或 `tests/e2e/**` 不能单独决定层级。
...
10. 两个生产 schema 的仓库必须提供 `openspec/agent-runtime/scripts/validate_tasks_quality.py`。`openspec/agent-runtime/scripts/write_evidence_ledger.py` 和 `openspec/schemas/shared/evidence-ledger.schema.json` 用于可选 audit ledger。生成或修订 `tasks.md` 后必须运行 `python3 openspec/agent-runtime/scripts/validate_tasks_quality.py openspec/changes/<change-slug>/tasks.md`。若 validator 缺失，先同步 bundled schema/runtime；缺失本身是 schema sync blocker。若报告 error，先修订 `tasks.md`；OpenSpec 结构校验通过不等同于 Testing Quality Core 通过。
