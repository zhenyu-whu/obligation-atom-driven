...
10. 两个生产 schema 的仓库必须提供 `openspec/agent-runtime/scripts/validate_tasks_quality.py`。生成或修订 `tasks.md` 后必须运行 `python3 openspec/agent-runtime/scripts/validate_tasks_quality.py openspec/changes/<change-slug>/tasks.md`。若脚本缺失，先同步 bundled schema/runtime；脚本缺失本身是 schema sync blocker。若报告 error，先修订 `tasks.md`；OpenSpec 结构校验通过不等同于 Testing Quality Core 通过。
