...
14. 每个 Test ID 必须使用 exact `T-###`（三位数字，例如 `T-001`，可用 `T-000` 表示 repository/runtime support），在整个 change 内唯一且只归属一个 AC。禁止把 AC 编号、测试名称、slug、字母后缀或描述性文本写入 Test ID；语义应写入 `Test File / Name`、`Layer`、`Covers Rows`、`Evidence Produced` 或 optional ledger。
15. Testing Quality Core 不可选：`Test Layer Plan`、`Test Evidence Matrix` 和 `Regression Test Deposit` 必须始终存在。tasks/propose 阶段只生成测试计划契约，不生成或宣称新 required behavior 的测试代码已经 red/green 完成；新增/变更 behavior 默认应是 `red-required`，`Observed Red Failure` / `Evidence Produced` 可写 apply-pending 占位。每个 required behavior Test ID 必须有 red/green TDD fields，除非写明 source-backed `not-applicable` 或 `blocked` 理由；不得把 smoke/browser proof 当作 unit/component/API/DB/security 等可低层稳定断言的替代品。固定命令必须是真正定位该 Test ID 的最小命令，不得用 `pnpm test* -- ...` 透传 file/filter 作为 selector。`Test Evidence Matrix` 必须写明 `Default Path Level` 和预期 layer-specific harness；具体测试代码是否满足 harness/runtime 由 apply/evidence/final gate 检查。
...
17. tasks 必须定义每个 AC 的 execution evidence expectation，包括 commands、browser/rendered artifacts、API/DB/job/storage/log/audit facts、runner/CI result/report 和 default-production-path proof。Optional audit ledger 只能汇总这些事实；若生成，必须使用机器字段：`acId`、`behaviorContract`、`assertionOracle`、red/green/regression result、`defaultPathFacts`、`fixtureBoundary`、`tddStatus` 和 `notApplicableReason`。
...
8. `production-obligation-atom-driven` / `tasks.md`：三张 coverage 表完整且 projection-aware；`Obligation Atom Coverage` 单行单 GA；所有 task ID 引用都能解析到 checkbox；每个 Test ID 都匹配 exact `T-[0-9]{3}` 且 canonical evidence directory 使用同名最后一级目录；每个 AC 有 final verification checkbox；每个 AC 有 execution evidence expectation；每个 required behavior Test ID 都有 `Test Layer Plan` layer decision、TDD red/green fields 和 `Regression Test Deposit` 永久回归沉淀或 source-backed 不适用理由；每个 AC 有 Prerequisites/Provides/Consumes/Start Gate；AC section 顺序满足 runtime provision graph；无 GA ranges、无 aggregate row、无 orphan direct atom、无 projection mismatch、无后置 provider dependency、无 implementation-detail tests 作为 required behavior oracle。
9. `production-default-acceptance-driven` / `tasks.md`：三张 coverage 表完整且 handling-aware；`Scope Item Coverage` 单行单 `SI-###`；所有 task ID 引用都能解析到 checkbox；每个 Test ID 都匹配 exact `T-[0-9]{3}` 且 canonical evidence directory 使用同名最后一级目录；每个 AC 有 final verification checkbox；每个 AC 有 execution evidence expectation；每个 required behavior Test ID 都有 `Test Layer Plan` layer decision、TDD red/green fields 和 `Regression Test Deposit` 永久回归沉淀或 scope-backed 不适用理由；每个 AC 有 Prerequisites/Provides/Consumes/Start Gate；AC section 顺序满足 runtime provision graph；无 SI ranges、无 aggregate row、无 orphan scope item、无 handling mismatch、无后置 provider dependency、无 implementation-detail tests 作为 required behavior oracle。
10. 两个生产 schema 的仓库必须提供 `openspec/agent-runtime/scripts/validate_tasks_quality.py`。`openspec/agent-runtime/scripts/write_evidence_ledger.py` 和 `openspec/schemas/shared/evidence-ledger.schema.json` 用于可选 audit ledger。生成或修订 `tasks.md` 后必须运行默认 plan gate：`python3 openspec/agent-runtime/scripts/validate_tasks_quality.py openspec/changes/<change-slug>/tasks.md`。若 validator 缺失，先同步 bundled schema/runtime；缺失本身是 schema sync blocker。若报告 error，先修订 `tasks.md`；OpenSpec 结构校验通过不等同于 Testing Quality Core 通过。不要在 propose/tasks 阶段补造 green evidence 或 ledger；这些属于 apply 阶段。
