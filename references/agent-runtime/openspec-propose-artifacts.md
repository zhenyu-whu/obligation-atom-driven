...
17. tasks 必须定义每个 AC 的 execution evidence expectation，包括 commands、`command.log` 或 runner/CI result/report、browser/rendered artifacts、API/DB/job/storage/log/audit facts，以及 default-production-path proof；测试 artifact 通过 runner/CI result/report、框架 output directory 或记录的 artifact directory 暴露，由 worker、final audit 或 CI collector 收集到 canonical directory。
