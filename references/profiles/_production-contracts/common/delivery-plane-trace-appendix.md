...
- `trace/manifest.json` 必须列出每个 artifact 的 artifact path、trace path 和 trace digest。
...
- `verification.md` 主体必须包含 `Verification Intent`、`Proof Slice Matrix`、`Layer / Harness / Fixture Notes`、`Do Not Test`；`runtime-coverage-reconciliation` 和 `slice-consistency-checklist` 必须写入 `trace/verification.trace.json`。
