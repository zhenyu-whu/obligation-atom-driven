...
- 每个 artifact trace 必须包含 renderer 所需的 `delivery-plane` payload；缺失 payload 会导致 renderer 生成失败，但 static validator 不再把 Markdown exact output comparison 作为门禁。
...
- `trace/manifest.json` 如保留，只是非权威 registry/version metadata，可列出当前阶段已生成 artifact 的 artifact path、trace path 和 trace schema。下游不得通过 manifest registry metadata 解析上游语义；`render-contract-version` 不参与 propose/apply/archive gate。
- renderer pure function 只用于生成给人看的 Markdown artifact 和 Trace Appendix；static validator 不再比较 Markdown artifact 与 renderer exact output，Markdown drift 不产生 `VAL-*-RENDER-*` warning 或 hard error。
