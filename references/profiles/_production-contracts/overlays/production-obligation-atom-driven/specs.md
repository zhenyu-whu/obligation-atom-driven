...
- 当整个 proposal register 中没有任何 `spec-requirement` / `spec-guard` direct atom 时，必须创建 `trace/specs/no-spec-delta/README.trace.json` 作为 canonical no-delta specs completion，并由 renderer 生成 `specs/no-spec-delta/README.md` marker。该 marker 只允许 `production-obligation-atom-driven` 使用，且不得与任何正常 `specs/<capability>/spec.md` 共存。
