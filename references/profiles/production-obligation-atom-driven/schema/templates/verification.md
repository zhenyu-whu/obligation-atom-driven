...
<!--
每个 required / preserve / proof-only runtime row 必须列出 expected Proof Slice。
Coverage Status = covered 仅当 Missing Proof Slice IDs = None。
manual / not-applicable 必须通过对应 runtime row 的 source-backed scope role、default path、external boundary 或 no-scope boundary 解释。
-->
...
| --------------- | --------------------------------------------------------------- | --------------------------------------------------------------------------- | --------------------------------- | ------------------------------- | -------------------------------------------------------------- | ----------------------------------------------------------------------- |
| <!-- OP-001 --> | <!-- required / preserve / proof-only / no-scope / external --> | <!-- primary / supporting / preserve / proof-only / no-scope / external --> | <!-- PS-001, PS-002 或 None。 --> | <!-- None 或缺失的 PS-###。 --> | <!-- covered / partial / manual / not-applicable / blocked --> | <!-- None；或 source-backed/manual/environment/not-covered reason。 --> |

### Slice Consistency Checklist

- [ ] 每个 Proof Slice 只引用 runtime-acceptance.md 中已定义的 Runtime Row IDs。
- [ ] 每个 Primary Runtime Row ID 都包含在同一行 Runtime Row IDs 中。
- [ ] 每个 Proof Slice 只有单一 Primary Runtime Row ID、Primitive Type、Branch / Variant、Primary Layer、Production Owner 和 Primary Assertion Shape。
- [ ] 每个 Proof Slice 只覆盖一个独立可失败分支；没有合并 edit/add/delete、replay/mismatch、success/failure、retryable/non_retryable/empty_result/timeout、多个 viewport、多个日志类别、多个 redaction 类别或多个 auth/security negative branch。
- [ ] 每个 Production Owner 都是 exactly one production code boundary token，不是 owner list、测试目录、runner 入口或 evidence 目录。
- [ ] 每个 required / preserve / proof-only runtime row 的所有独立分支都有 expected Proof Slice。
- [ ] Runtime Coverage Reconciliation 中 `Coverage Status = covered` 的行，其 expected Proof Slice IDs 全部存在且均为原子 slice，`Missing Proof Slice IDs = None`。
- [ ] 没有在 verification.md 中新增 source mapping 或重新解释 source/scope。
- [ ] 没有从 tasks.md、实现、测试文件结构或 evidence/deposit 结构反推 Proof Slice。
