...
Verification gate 依据 `openspec/schemas/shared/verification-regression-gates.md`；tasks 阶段只定义 `Verification Expectation`、唯一固定命令和计划态 `Evidence Status`。apply 阶段通过 evidence harness 执行同一个 `Fixed Command`，保存 `command.log`、harness-owned `ledger.json`、runner/CI result/report 和被索引的 artifacts；实际 `Evidence Produced` / `Evidence Status=passed` 由 apply 阶段填写。
`ledger.json` 是 apply-stage evidence harness 或 CI artifact collector 的输出；canonical evidence path、`command.log` 和 `ledger.json` 由 harness 管理。截图、trace、DOM/API/DB/job/log facts 应通过 runner artifact/attachment、框架 output directory 或 harness 注入的临时 artifact directory 产出，再由 harness 复制和索引。
...
最终完成不是表格声明：每个完成的 Test ID 必须有当前 worktree 的 execution evidence、canonical `command.log` 或 runner/CI result/report、harness-owned `ledger.json` 或等价 CI artifact index、AC evidence audit 通过，以及全量 Testing Quality Core final audit 通过。`Evidence Status = blocked` 只是无法完成验证的证据状态，不是成功收口；fixed command/typecheck/lint/test/evidence 缺口失败默认由 worker 修复并重跑，只有无法自主解除时才形成流程级 blocker。
...
| ------------------------------------------------------------------------------- | ---------------------------------------------------------- | -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- | -------------------------- | ------------------------------------------------------------------ | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- | ------------------------- | --------------------- | --------------- | ------------------------------------ |
| <!-- T-001；必须匹配 exact T-###，禁止 T-005B / T-AC005-... / T-001-smoke。 --> | <!-- AC-001；只能有一个 AC ID，禁止多个 AC 或 shared。 --> | <!-- 唯一固定命令；apply/evidence/final gate 都通过 evidence harness 执行这个命令。不得用 `pnpm test -- <file> -t <name>` 或 `pnpm test:e2e -- ...` 透传 selector；需要 filter 时用 `pnpm exec vitest ... <file> -t <name>`、Playwright 直接命令或专用 script。 --> | <!-- test file / stable test name or filter；永久测试代码按 production owner 放置：unit/domain -> packages/<domain>/src/**，DB -> packages/db/**，API route -> apps/web/app/api/**，component -> 对应 app/package，browser E2E -> tests/e2e/**，runtime/ops guard -> tests/runtime/**；禁止 tests/<change-slug-prefix>/**、openspec/changes/** 或 openspec-results/** / test-results/**。canonical evidence path、command.log 和 ledger.json 由 evidence harness/CI collector 管理；测试文件通过 runner attachment/report 或 harness 配置输出目录暴露 artifacts。同一文件覆盖多层时用不同 Test ID + stable filter 拆行。 --> | <!-- 只能写单一层级：unit / component / route/API contract / DB integration / worker/job integration / realtime/SSE integration / browser E2E / visual/responsive / security/negative / config/ops/check。 --> | <!-- RS-001, OP-001, ST-001, CH-001。 --> | <!-- http-app / route-handler-real-service / component-user-flow / db-integration / service-contract / controller-contract / browser-e2e / static-analysis；controller-contract 涉及 route/auth/query/tenant/DI 时必须配对 http-app 或 route-handler-real-service Test ID。 --> | <!-- mocked/sandboxed segments and paired default-path proof。 --> | <!-- 预期验证的行为契约，必须来自 GA/spec/design/runtime row，不是测试错误。 --> | <!-- tasks 阶段默认 planned；apply 完成后使用 passed / not-applicable / blocked。 --> | <!-- None / T-000；只能引用 earlier AC 的 Test ID。 --> | <!-- `openspec-results/<change-slug>/AC-001/T-001/`；最后一级目录必须与 Test ID 完全一致。 --> | <!-- tasks 阶段列预期证据类型，例如 harness-owned ledger.json、command.log、runner/CI result/report、screenshot、trace、API/DB/job/log facts；apply 阶段由 evidence harness 填写实际 artifact paths。 --> | <!-- yes + root/package/CI entry；或 no + blocker reason。 --> | <!-- GA/spec/design。 --> | <!-- projection。 --> | <!-- role。 --> | <!-- 不引入 source 外 behavior。 --> |
...
永久测试代码承载行为断言和稳定 regression entry；canonical `ledger.json` 由 apply-stage evidence harness、final verification task 或 CI artifact collector 生成。
