...
canonical evidence path、`command.log` 或 runner/CI result/report 由 worker/automation 管理。截图、trace、DOM/API/DB/job/log facts 应通过 runner artifact/attachment、框架 output directory 或记录的 artifact directory 产出，再由 worker/automation 复制和保存。
...
最终完成不是表格声明：每个完成的 Test ID 必须有当前 worktree 的 execution evidence、canonical `command.log` 或 runner/CI result/report、必需 artifacts/facts、AC evidence audit 通过，以及全量 Testing Quality Core final audit 通过。若使用等价 CI evidence，必须有可审计的 CI result/report。`Evidence Status = blocked` 只是无法完成验证的证据状态，不是成功收口；fixed command/typecheck/lint/test/evidence 缺口失败默认由 worker 修复并重跑，只有无法自主解除时才形成流程级 blocker。
