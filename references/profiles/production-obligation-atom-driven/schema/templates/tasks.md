...
区分 Artifact/Spec Owner AC 与 Runtime/Test Evidence Owner AC：两者不同时，本节只记录 handoff；AC-local Test IDs 和 checkbox Test IDs 仍必须留在 Test Evidence Matrix.AC ID 指定的 owning AC。
-->
...
| ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------ | -------------------- | ----------------------------------------------------- | ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| <!-- GA-0001；每行只能有一个 ID，不使用 GA-0001-GA-0010 这类 ranges，也不把多个 GA 放一格。 --> | <!-- spec-requirement / spec-guard / design-obligation / verification-obligation / contextual-only --> | <!-- 概述 source-backed obligation / preserve / stable negative boundary。 --> | <!-- AC-001；若 artifact/spec owner 与 runtime proof owner 不同，列出相关 AC 并在 proof 写 handoff。 -->      | <!-- AC-001.1, AC-001.2；必须是实际 checkbox ID，通常属于 artifact/spec owner。 --> | <!-- AC-003.5；必须是实际 checkbox ID，必须属于 runtime/Test Evidence owner。 --> | <!-- user interaction、API test、data assertion、worker/realtime path、security check、rendered layout check 等 proof summary；跨 AC 时说明 handoff。 --> |
...
| ------------------------ | ------------------------------- | ---------------------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | -------------------- | ----------------------- | --------------------- | -------------------------------------------------- |
| <!-- capability name --> | <!-- exact requirement name --> | <!-- exact scenario name --> | <!-- GA-0001, GA-0002；逐个枚举 exact IDs，不使用 ranges。 --> | <!-- spec-requirement scenario；或说明 spec-guard/design/verification atom 不伪造 scenario。 --> | <!-- AC-001；跨 AC proof 时列出 artifact/spec owner 与 runtime owner。 -->      | <!-- AC-001.1 -->       | <!-- AC-003.5；verification task 必须属于 Test Evidence owner。 -->     | <!-- scenario-level observable proof summary；跨 AC 时说明 handoff。 --> |
...
| ---------------------------------------------------- | --------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- | -------------------- | ----------------------- | --------------------- | ------------------------------------------ |
| <!-- exact design section / decision / gate item --> | <!-- material implementation、preservation 或 verification obligation。 --> | <!-- GA-0001, GA-0002, 或 Not applicable；逐个枚举 exact IDs，不使用 ranges。 --> | <!-- design-obligation / verification-obligation / spec-guard handling。 --> | <!-- AC-001；跨 AC proof 时列出 artifact/spec owner 与 runtime owner。 -->      | <!-- AC-001.1 -->       | <!-- AC-003.5；verification task 必须属于 Test Evidence owner。 -->     | <!-- design-obligation proof summary；跨 AC 时说明 handoff。 --> |

## Runtime Acceptance Index

<!--
本节是主 agent preflight 的轻量路由表，不是 executable work，也不是 runtime/test 详情来源。
它只把 AC 路由到 Appendix 中的 row IDs、Test IDs 和 AC dependency graph；不要在这里重复 fixed commands、证据目录、fixture 或 row 详情。
...
Runtime/Test Evidence ownership 以本节和 Test Evidence Matrix 为准；coverage table 的跨 AC 引用不能把 Test ID 重新分配给 artifact/spec owner AC。
-->
...
- <!-- T-001, T-002；只列本 AC 拥有的 Test IDs，必须匹配 exact T-###；固定命令、证据目录和 fixture 明细只在 Test Evidence Matrix 中定义。 -->
- <!-- 如果本 AC 只是 artifact/spec owner，而 runtime proof 由其他 AC 拥有，不要列其他 AC 的 Test ID；在 Required Evidence / coverage row 写 handoff summary。 -->
...
- Test evidence: <!-- T-001, T-002；Test IDs 必须匹配 exact T-###；fixed commands、evidence directories 和 execution artifacts 只在 Test Evidence Matrix 中维护。 -->
- Browser / rendered evidence: <!-- screenshot / DOM / responsive / a11y / interaction evidence 的类型和验收意图；具体 artifact path 由对应 Test ID 的 Evidence Produced 记录。 -->
...
- <!-- AC-level default path / mock 原则摘要；fixture 只能提供输入、凭证/claims、外部返回或确定性环境，不能替代被测 runtime boundary 的默认实现；逐 Test ID 的 fixture boundary 只在 Test Evidence Matrix 中定义。 -->

Mock Policy:

- <!-- 哪些允许 sandbox/mock；哪些必须走 default production wiring；route/auth/session/provider/storage/queue/env/config 等 default path 不得只在 test-only resolver 或 NODE_ENV=test 下成立。 -->
...
- [ ] AC-001.1 <!-- 用中文描述此 acceptance slice 下的具体实现或验证任务。 -->
      ...
      Test IDs: <!-- 本 task 建立、维护或由 final verification 执行的 T-### row IDs；只能列 Test Evidence Matrix 中 AC ID 等于本 AC 的 Test IDs，不得带名称后缀，不得重复 fixed command/path。 -->
...
<!-- 每个 AC section 必须至少包含一个 final verification / acceptance checkbox，例如 AC-001.N。该 checkbox 必须出现在本节 Required Evidence、coverage tables 的 Verification Task IDs 和最终 apply execution evidence 中。若 coverage proof handoff 到其他 AC，该 Verification Task ID 应引用 runtime/Test Evidence owner AC 的 final verification checkbox。 -->
...
Test Evidence Matrix 是 fixed command、test file/name、evidence directory、expected execution artifacts、fixture boundary 和 CI runnable 状态的唯一事实来源；实际执行结果由 apply 阶段更新。
Test Layer Plan 是分层测试体系的唯一事实来源；不要只用一个 smoke/browser proof 覆盖可低层稳定断言的规则、表单状态、API contract、DB invariant 或安全边界。
Verification gate 依据 `openspec/schemas/shared/verification-regression-gates.md`；tasks 阶段只定义 `Verification Expectation`、唯一固定命令和计划态 `Evidence Status`。apply 阶段运行同一个 `Fixed Command`，保存 `command.log` 或 runner/CI result/report 以及需要保留的 artifacts；实际 `Evidence Produced` / `Evidence Status=passed` 由 apply 阶段填写。
canonical evidence path、`command.log` 或 runner/CI result/report 由 worker、change-stabilizer 或 automation 管理。截图、trace、DOM/API/DB/job/log facts 应通过 runner artifact/attachment、框架 output directory 或记录的 artifact directory 产出，再由 worker、change-stabilizer 或 automation 复制和保存。
...
最终完成不是表格声明：每个完成的 Test ID 必须有当前 worktree 的 execution evidence、canonical `command.log` 或 runner/CI result/report、必需 artifacts/facts、AC evidence audit 通过、change-stabilizer 全局收敛完成，以及只读 final-reviewer 返回 pass。若使用等价 CI evidence，必须有可审计的 CI result/report。`Evidence Status = blocked` 只是无法完成验证的证据状态，不是成功收口；fixed command/typecheck/lint/test/evidence 缺口失败默认由 worker 或 change-stabilizer 修复并重跑，只有无法自主解除时才形成流程级 blocker。若 final-reviewer 在 stabilization 后仍返回 blocker，apply 必须停止并交人工查验。
...
<!--
测试层级代码要求摘要（apply 阶段执行；tasks 阶段只据此选择层级和默认路径，不声明代码已满足）：
...
| ------------------------------------------------------------------------------- | ---------------------------------------------------------- | -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- | -------------------------- | ------------------------------------------------------------------ | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- | ------------------------- | --------------------- | --------------- | ------------------------------------ |
| <!-- T-001；必须匹配 exact T-###，禁止 T-005B / T-AC005-... / T-001-smoke。 --> | <!-- AC-001；只能有一个 AC ID，禁止多个 AC 或 shared。 --> | <!-- 唯一固定命令；apply/stabilizer/final-review gate 都运行这个命令。不得用 `pnpm test -- <file> -t <name>` 或 `pnpm test:e2e -- ...` 透传 selector；需要 filter 时用 `pnpm exec vitest ... <file> -t <name>`、Playwright 直接命令或专用 script。 --> | <!-- test file / stable test name or filter；永久测试代码按 production owner 放置：unit/domain -> packages/<domain>/src/**，DB -> packages/db/**，API route -> apps/web/app/api/**，component -> 对应 app/package，browser E2E -> tests/e2e/**，runtime/ops guard -> tests/runtime/**；禁止 tests/<change-slug-prefix>/**、openspec/changes/** 或 openspec-results/** / test-results/**。canonical evidence path、command.log 或 runner/CI result/report 由 worker、change-stabilizer 或 automation 管理；测试文件通过 runner attachment/report 或配置输出目录暴露 artifacts。同一文件覆盖多层时用不同 Test ID + stable filter 拆行。 --> | <!-- 只能写单一层级：unit / component / route/API contract / DB integration / worker/job integration / realtime/SSE integration / browser E2E / visual/responsive / security/negative / config/ops/check。 --> | <!-- RS-001, OP-001, ST-001, CH-001。 --> | <!-- http-app / route-handler-real-service / component-user-flow / db-integration / service-contract / controller-contract / browser-e2e / static-analysis；controller-contract 涉及 route/auth/query/tenant/DI 时必须配对 http-app 或 route-handler-real-service Test ID。 --> | <!-- fixture 只能提供输入/凭证/外部返回/确定性环境；列出 mocked/sandboxed segments，以及 production-compatible default export/dependency/config 由哪个 proof 覆盖。 --> | <!-- 预期验证的行为契约，必须来自 GA/spec/design/runtime row，不是测试错误。 --> | <!-- tasks 阶段默认 planned；apply 完成后使用 passed / not-applicable / blocked。 --> | <!-- None / T-000；只能引用 earlier AC 的 Test ID。 --> | <!-- `openspec-results/<change-slug>/AC-001/T-001/`；最后一级目录必须与 Test ID 完全一致。 --> | <!-- tasks 阶段列预期证据类型，例如 command.log、runner/CI result/report、screenshot、trace、API/DB/job/log facts；apply 阶段填写实际 artifact paths。 --> | <!-- yes + root/package/CI entry；或 no + blocker reason。 --> | <!-- GA/spec/design。 --> | <!-- projection。 --> | <!-- role。 --> | <!-- 不引入 source 外 behavior。 --> |
...
每个 required behavior Test ID 完成时必须为 deposited；required 只表示计划，不能作为完成状态。blocked / not-applicable 是无法完成验证或不适用的审计状态，不是 required behavior 的成功完成状态；只有无法由 agent 在当前 change 范围内自主解除时才写 blocked。
deposited 表示永久测试文件或稳定 smoke/e2e/ops 入口已存在，regression command 已由当前 execution evidence 证明可重跑；不能只登记计划路径或一次性 evidence-only 文件。
永久测试文件必须按 production owner 放置，不得按 change slug 聚合；`openspec-results/<change-slug>/...` 只保存 evidence，不保存测试代码。
永久测试代码承载行为断言和稳定 regression entry；canonical evidence directory 只保存执行证据，不保存永久测试代码。
