...
- Test evidence: <!-- T-001, T-002；Test IDs 必须匹配 exact T-###；fixed commands、evidence directories、execution artifacts 和 optional ledger files 只在 Test Evidence Matrix 中维护。 -->
- Browser / rendered evidence: <!-- screenshot / DOM / responsive / a11y / interaction evidence 的类型和验收意图；具体 artifact path 由对应 Test ID 的 Evidence Produced / optional ledger 记录。 -->
...
- [ ] AC-001.1 <!-- 用中文描述此 acceptance slice 下的具体实现或验证任务。 -->
      ...
      Test IDs: <!-- 本 task 建立、维护或由 final verification 执行的 T-### row IDs；只能列 Test Evidence Matrix 中 AC ID 等于本 AC 的 Test IDs，不得带名称后缀，不得重复 fixed command/path/optional ledger。 -->
...
Test Evidence Matrix 是 fixed command、test file/name、evidence directory、expected execution artifacts、optional ledger file、fixture boundary 和 CI runnable 状态的唯一事实来源；实际 red/green 执行结果由 apply 阶段更新。
Test Layer Plan 是分层测试体系的唯一事实来源；不要只用一个 smoke/browser proof 覆盖可低层稳定断言的规则、表单状态、API contract、DB invariant 或安全边界。
TDD red/green gate 依据 `openspec/schemas/shared/tdd-regression-gates.md`；tasks 阶段只定义 `Expected Red Failure` 和固定命令，`Observed Red Failure` / 实际 `Evidence Produced` / `TDD Status=green-passed` 由 apply 阶段在运行 red/green 后填写。
...
| ------------------------------------------------------------------------------- | ---------------------------------------------------------- | -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- | -------------------------- | ------------------------------------------------------------------ | -------------------------------------------------------------------- | ------------------------------------------------------------------------------ | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------ | -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- | -------------------------------------------------------------- | ------------------------- | --------------------- | --------------- | ------------------------------------ |
| <!-- T-001；必须匹配 exact T-###，禁止 T-005B / T-AC005-... / T-001-smoke。 --> | <!-- AC-001；只能有一个 AC ID，禁止多个 AC 或 shared。 --> | <!-- 最小固定命令；不得用 `pnpm test -- <file> -t <name>` 或 `pnpm test:e2e -- ...` 透传 selector；需要 filter 时用 `pnpm exec vitest ... <file> -t <name>`、Playwright 直接命令或专用 script。 --> | <!-- test file / stable test name or filter；永久测试代码按 production owner 放置：unit/domain -> packages/<domain>/src/**，DB -> packages/db/**，API route -> apps/web/app/api/**，component -> 对应 app/package，browser E2E -> tests/e2e/**，runtime/ops guard -> tests/runtime/**；禁止 tests/<change-slug-prefix>/**、openspec/changes/** 或 test-results/**。同一文件覆盖多层时用不同 Test ID + stable filter 拆行。 --> | <!-- 只能写单一层级：unit / component / route/API contract / DB integration / worker/job integration / realtime/SSE integration / browser E2E / visual/responsive / security/negative / config/ops/check。 --> | <!-- RS-001, OP-001, ST-001, CH-001。 --> | <!-- http-app / route-handler-real-service / component-user-flow / db-integration / service-contract / controller-contract / browser-e2e / static-analysis；controller-contract 涉及 route/auth/query/tenant/DI 时必须配对 http-app 或 route-handler-real-service Test ID。 --> | <!-- mocked/sandboxed segments and paired default-path proof。 --> | <!-- yes/no + expected failing gap；required behavior 默认 yes。 --> | <!-- red 阶段固定命令；通常与 Fixed Command 相同，必须可从仓库根目录重跑且真实定位本 Test ID。 --> | <!-- 预期失败缺口，必须来自 GA/spec/design/runtime row，不是测试错误。 --> | <!-- apply 阶段填写；tasks 阶段写 Pending apply red run，evidence/final gate 必须替换为真实失败摘要和 evidence path。 --> | <!-- green 阶段重跑命令；通常与 Fixed Command 相同。 --> | <!-- tasks 阶段默认 red-required；apply 完成后使用 red-observed / green-passed / not-applicable / blocked，禁止 red-green 等非枚举状态。 --> | <!-- None / T-000；只能引用 earlier AC 的 Test ID。 --> | <!-- `test-results/<change-slug>/AC-001/T-001/`；最后一级目录必须与 Test ID 完全一致。 --> | <!-- tasks 阶段列预期证据类型，例如 command.log 或 runner/CI result/report；apply 阶段替换或补充实际 artifact paths、green-result.json/regression-result.json、DOM/screenshot、trace、API/DB/job/log facts。optional ledger 可列为 audit receipt，但不能替代执行证据。 --> | <!-- 可留空；若生成 optional audit ledger，写 `test-results/<change-slug>/AC-001/T-001/ledger.json`，字段名使用 acId，不使用 ac。Ledger File 非空时 evidence/final gate 要求文件存在。 --> | <!-- yes + root/package/CI entry；或 no + blocker reason。 --> | <!-- GA/spec/design。 --> | <!-- projection。 --> | <!-- role。 --> | <!-- 不引入 source 外 behavior。 --> |
