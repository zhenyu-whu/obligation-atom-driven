...
最终完成不是表格声明：每个完成的 Test ID 必须有当前 worktree 的 execution evidence、canonical `command.log` 或 runner/CI result/report、harness-owned `ledger.json` 或等价 CI artifact index、AC evidence audit 通过，以及全量 Testing Quality Core final audit 通过或明确 blocker。
