...
`openspec-apply-change` 阶段负责落地测试、实现、命令执行和 evidence 更新：根据 `Test Evidence Matrix` 建立或补齐适用测试，完成实现后通过 apply-stage evidence harness 运行同一个 `Fixed Command`，把 `command.log`、harness-owned `ledger.json`、runner/CI result/report 和需要保留的 artifact 保存到 canonical evidence directory，并更新 `Evidence Status`、`Evidence Produced` 和 `Regression Test Deposit` 状态。
...
## Evidence Harness And Ledger Ownership

- `ledger.json` 是 OpenSpec apply-stage execution ledger，由 evidence harness、final verification task 或等价 CI artifact collector 在 `Fixed Command` 外层生成和维护；测试代码通过 runner artifacts/report 或 harness-provided output directory 提供输入事实。
- evidence harness 必须在执行前清理或隔离目标 `Evidence Directory`，捕获 `Fixed Command` 的 stdout/stderr 到 `command.log`，记录命令、退出码、开始/结束时间、工作目录、Test ID、AC ID、change slug、runner report paths、artifact paths 和 evidence status，并在命令结束后写入 harness-owned `ledger.json`。
- canonical evidence path、`command.log`、`ledger.json` 和 OpenSpec change/AC/Test routing 由 harness 配置和注入。测试代码关注行为断言；需要保留的截图、trace、DOM snapshot、API/DB/job/log facts 应通过 runner attachment/report、测试框架 output directory，或由 harness 注入的临时 artifact directory 产出，再由 evidence harness 复制、索引到 canonical directory。
- 如果测试框架没有 attachment/report 机制，可以使用共享测试 fixture 写临时 machine-readable artifact；该 fixture 由 evidence harness 配置输出目录，测试文件只消费该目录配置，不写 change slug、AC ID、Test ID 或 canonical OpenSpec evidence path。
- `ledger.json` 中的结构化事实必须来自 runner report、artifact 文件、命令环境和可审计 readback，避免成为与测试断言脱钩的验收叙述。若 ledger 与 `command.log`、runner report 或 artifact 不一致，以可重跑命令和 runner/artifact 事实为准，并重新生成或修正 harness evidence。
- AC evidence gate 和 final gate 不得只检查 `ledger.json` 存在；必须同时检查固定命令的当前执行结果、`command.log` 或 runner/CI result/report、artifact 路径存在性、时间/运行标识非陈旧，以及 `Evidence Produced` 与实际落盘内容一致。
...
- `passed`：固定命令或等价 runner/CI result/report 已在当前 worktree 通过并落盘；本地 apply evidence 应包含 harness-owned `ledger.json`，等价 CI evidence 必须有可审计的 CI result/report 和 artifact index。
...
`Evidence Status = blocked` 是 Test Evidence row 的证据状态，不等同于 worker/apply flow blocker。`Fixed Command` 失败、typecheck/lint/test 失败、缺失 `command.log` / `ledger.json` / runner artifact、测试 oracle 不足或 artifact/evidence 未闭合，默认都是当前 AC 尚未完成的修复工作；worker 必须先修实现、测试、artifacts 或 evidence harness 并重跑固定命令。只有当失败原因无法由 agent 在当前 change 范围内自主解除时，才可把 evidence 标记为 `blocked` 并返回流程级 blocker。
...
- 本地 apply evidence 必须包含 harness-owned `ledger.json`；等价 CI evidence 必须有可审计的 runner/CI artifact index。
