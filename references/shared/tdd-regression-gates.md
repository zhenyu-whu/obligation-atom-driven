...
`ledger.json` 是可选的 apply/reviewer/archive 审计收据，用于把 `tasks.md`、执行命令、result evidence 和 regression deposit 汇总到一个机器可验对象。普通测试命令不要求自然生成 OpenSpec 专用 ledger；当前 worktree 的 `command.log`、runner/CI report、`green-result.json`、`regression-result.json` 等执行产物才是完成 Test ID 的事实源。
