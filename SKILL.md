...
- `references/profiles/production-obligation-atom-driven/schema/schema.yaml`: v12 projection-aware schema that consumes canonical final change packets and `obligation-atom-index.md` directly from `openspec/orchestrate`, creates proposal, specs, design, runtime-acceptance, verification, and tasks artifacts whose bodies are Delivery Plane contracts, moves coverage/projection/reconciliation tables to terminal `Trace Appendix` sections, keeps proposal/design Delivery Plane free of coverage leakage, requires tasks AC sections to include resolved runtime contract excerpts, distinguishes `spec-requirement`, `spec-guard`, `design-obligation`, `verification-obligation`, and `contextual-only` atoms, requires `runtime-acceptance.md` to define canonical runtime rows with upstream runtime coverage mapping, requires `verification.md` to split required/preserve/proof-only runtime rows into single-layer atomic Proof Slices without extra business-oracle grouping IDs, and does not use `source-truth.md`, separate `acceptance.md`, legacy source coverage artifacts, `change-source-map.md`, or pre-proposal source artifacts.
...
- `references/profiles/production-default-acceptance-driven/schema/schema.yaml`: v8 default-style schema that uses proposal, specs, design, runtime-acceptance, verification, and tasks artifacts whose bodies are Delivery Plane contracts; it does not consume orchestrate packets or global indexes, uses change-local `SI-###` scope items only for lightweight cross-artifact coverage in terminal `Trace Appendix` sections, keeps proposal/design Delivery Plane free of coverage leakage, requires tasks AC sections to include resolved runtime contract excerpts, requires `runtime-acceptance.md` to define canonical runtime rows with upstream runtime coverage mapping, and requires `verification.md` to split required/preserve/proof-only runtime rows into single-layer atomic Proof Slices without extra business-oracle grouping IDs.
