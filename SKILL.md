...
- `references/profiles/production-obligation-atom-driven/schema/schema.yaml`: v11 projection-aware schema that consumes canonical final change packets and `obligation-atom-index.md` directly from `openspec/orchestrate`, creates proposal, specs, design, runtime-acceptance, verification, and tasks artifacts whose bodies are Delivery Plane contracts, moves coverage/projection/reconciliation tables to terminal `Trace Appendix` sections, keeps proposal/design Delivery Plane free of coverage leakage, requires tasks AC sections to include resolved runtime contract excerpts, distinguishes `spec-requirement`, `spec-guard`, `design-obligation`, `verification-obligation`, and `contextual-only` atoms, requires `runtime-acceptance.md` to define canonical runtime rows, requires `verification.md` to split required VID oracle into single-layer Proof Slices, and does not use `source-truth.md`, separate `acceptance.md`, legacy source coverage artifacts, `change-source-map.md`, or pre-proposal source artifacts.
