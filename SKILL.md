...
- `production-obligation-atom-driven`: docs-driven Greenfield implementation schema. It consumes canonical change packets and `obligation-atom-index.md` from `openspec/orchestrate`, preserves `GA-####` IDs, and uses projection-aware proposal/spec/design/runtime-acceptance/verification/tasks artifacts. `runtime-acceptance.md` owns canonical runtime `RS-/OP-/ST-/CH-` rows, `verification.md` owns independent test intent, behavior oracle, and Proof Slice Matrix, and `tasks.md` owns production implementation plus runtime acceptance projection.
...
- `references/profiles/production-obligation-atom-driven/schema/schema.yaml`: projection-aware schema that consumes canonical final change packets and `obligation-atom-index.md` directly from `openspec/orchestrate`, creates proposal, specs, design, runtime-acceptance, verification, and acceptance-driven tasks, distinguishes `spec-requirement`, `spec-guard`, `design-obligation`, `verification-obligation`, and `contextual-only` atoms, requires `runtime-acceptance.md` to define canonical runtime rows, requires `verification.md` to split required VID oracle into single-layer Proof Slices, and does not use `source-truth.md`, separate `acceptance.md`, legacy source coverage artifacts, `change-source-map.md`, or pre-proposal source artifacts.
...
- `references/profiles/production-default-acceptance-driven/schema/schema.yaml`: default-style schema that uses proposal, specs, design, runtime-acceptance, verification, and acceptance-driven tasks; it does not consume orchestrate packets or global indexes, uses change-local `SI-###` scope items only for lightweight cross-artifact coverage, requires `runtime-acceptance.md` to define canonical runtime rows, and requires `verification.md` to split required VID oracle into single-layer Proof Slices.
